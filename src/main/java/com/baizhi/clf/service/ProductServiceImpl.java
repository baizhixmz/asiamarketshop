...
public class ProductServiceImpl implements ProductService {
	...
	public List<SproductEntity> findProductsByCategoryId(String adminId,
		...
		if (username.equals("SuperAdmin")) {
			// 如果是超级管理员
			List<SproductEntity> products = productDAO.selectProducts(
					sproductEntity, page, "flag='Y'");
		...
		// 如果不是超级管理员
		List<SadminProductEntity> adminProducts = adminRelationDAO
				.selectAdminProducts(adminId);

		// 符合条件的所有数据
		ArrayList<SproductEntity> products = new ArrayList<SproductEntity>();

		for (SadminProductEntity adminProduct : adminProducts) {
			SproductEntity product1 = productDAO.selectProductById(adminProduct
					.getProductId());
	...
	public List<SproductEntity> findProductsMain(String adminId,
		...
		if (username.equals("SuperAdmin")) {
			// 如果是超级管理员
			List<SproductEntity> products = productDAO.selectProducts(
					new SproductEntity(), new Page(1, 6), "flag='Y'");
		...
		for (int i = 0; i < index; i++) {
			...
			SproductEntity sproductEntity = productDAO
					.selectProductById(adminProducts.get(i).getProductId());
			// 覆盖价格为商铺自定义价格
	...
	public SproductEntity findProductDetail(String id) {
		...
		if (admin.getUsername().equals("SuperAdmin")) {
			...
			// 覆盖价格为自定义价格
			SadminProductEntity sadminProductEntity1 = adminRelationDAO
					.selectAdminProduct(sadminProductEntity);
	...
	public List<SproductEntity> findProductsByDsql(Page page, String condition) {
		...
		List<SproductEntity> sproductEntities = productDAO.selectProductByDsql(
				page, condition);
